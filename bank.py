...
class Bank:
    ...
    def getKeys(self):
        ...
        # Exercise
        return sorted(self.accounts.values(), key=lambda x: x.getName())
    
    def sort(self):
        """Returns the accounts alphabetically arranged by the account names"""
        return "\n".join(map(str, self.getKeys()))
...
def main(number = 10):
    """Creates and prints a bank"""
    bank = createBank(number)
    print(bank)

    # Prints the sorted accounts
    print("\nLet's sort:")
    print(bank.sort())
